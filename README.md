...
Each agent is one text file in `agents/`. To change how one behaves, edit its file. To change its AI model, edit the `model:` line at the top (`<provider>/<model>#variant`), then restart OpenCode. The same header also carries the `#variant` reasoning effort (how hard the model thinks) and `request.body.temperature` (how creative it is), shipped tuned per role: the builder and planner think at full depth, while the faster, cheaper jobs (tests, docs, UI) dial it down so they answer sooner. Full guide: [docs/AGENTS.md](docs/AGENTS.md).
