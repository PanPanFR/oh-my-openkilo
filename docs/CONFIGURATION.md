...
All five shipped plugins target the **opencode v2 plugin contract** (a `default` export of `{ id, setup }` registering through `ctx.tool.hook`, `ctx.event.subscribe`, and `ctx.session.hook`) and require **opencode >= 2.0**. On opencode 1.x they will not load; stay on pack v0.8.8 if you are still on v1. Hooks with no v2 equivalent (`chat.message`, `chat.params`, `config`) degrade with a single console warning naming every skipped hook. What remains active under v2: full event capture (`rtk` rewrite, `recall-first` gate, `graphify` reminder/poller, `agentmemory-capture` observations with lazy session init), tool hooks, caveman's session flag state, best-effort caveman reinforcement and agentmemory compaction context injection via the `context`/`compaction` session hooks.
