...
The example lists the 7 plugin specifiers the pack ships: 5 small files we own (`agentmemory-capture`, `recall-first`, `caveman`, `graphify`, `rtk`) plus 2 packages (`ponytail`, `superpowers`). Drop any you don't want to load — the pack degrades gracefully without them, the only required one being `agentmemory-capture` if you use the memory skills. `rtk` needs the `rtk` binary in PATH (token-saving bash rewrite); without it the plugin disables itself and commands pass through untouched. All 5 owned plugins are dual-contract (`{ id, server, setup }`): OpenCode v1 (needs >= 1.18.29 for object entrypoints) calls `server()`, OpenCode v2 calls `setup()`, so one file serves both hosts with identical behavior. Degraded on v2 only: `agentmemory` prompt/params/config capture and `caveman` in-session toggles have no v2 hook equivalent (event capture, file stash, session context, flag assertion keep working; a one-line console warning names the skipped hooks). The maintainer's live install additionally wires the third-party `i-have-adhd` plugin (ADHD response structure, always-on via flag file) with an absolute local path, so it stays out of this portable example; the `communication-style` rule already carries the one-line precedence (ADHD owns structure, Caveman owns density) for installs that add it.
