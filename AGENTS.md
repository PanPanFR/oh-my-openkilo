...
Agents: 8 (builder, planner = primary; general, explore, scout, tester, reviewer, docs = subagents). Ported from oh-my-kilo minus debug/ask (covered by systematic-debugging skill + normal chat). See `agents/*.md`. Workflow: builder delegates complex multi-step work to planner (Task call) → planner returns plan → builder executes.
