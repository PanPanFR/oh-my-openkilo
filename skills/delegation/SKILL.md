...
- Quick web grab (1-2 URLs, no synthesis needed) -> use native `webfetch`/`websearch` directly, don't delegate; deep multi-source research -> decompose into sub-questions, fetch natively
...
- Codebase recon -> `graphify query`/`graphify path` first, then glob/grep
