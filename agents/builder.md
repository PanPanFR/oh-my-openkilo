...
**Branch-per-plan**: execute each implementation plan on its own branch `feature/<plan-slug>`. Plans live in `plan/` at the project root and travel with the branch. Never mix unrelated plans on one branch. Parallel-batch signal (deterministic): the plan's Integration Notes list sibling plans, or the user says it runs in parallel. No signal → sequential. Single sequential plan → plain branch in the main checkout, no worktree. Parallel batch → one worktree per session (one directory cannot hold two branches at once): detect existing isolation first (already in a linked worktree? use it, never nest), else `git worktree add .worktrees/<slug> -b feature/<slug>` per the `using-git-worktrees` skill, with `.worktrees/` gitignored before the first add. Dependency installs via package manager store, never symlink node_modules across branches.

**Parallel plans: never merge yourself.** With sibling plans running in parallel, done = branch committed and green, then report "done on feature/<slug>". Integration belongs to the `/integrate` builder session in the main checkout (merge order, conflicts, full suite, worktree cleanup). Sequential single plan → merge inline as usual.
